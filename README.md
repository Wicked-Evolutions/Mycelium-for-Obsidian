...
`semantic_search` and `semantic_search_all` cannot claim an exact note total from capped embedding and FTS providers. Their `limit_reached` means at least one eligible provider candidate row was withheld; that candidate can be another indexed block from a note already represented in the results. Evidence-only provider sentinel rows never enter ranking, RRF scores, graph annotation, reranker input, or returned ordering.

`get_similar` excludes the reference file and its aliases, groups readable target files before applying its file limit, and retains each file's best similarity. Its `limit_reached` requires an additional qualifying distinct file. It reads candidates as needed rather than parsing every indexed note. The reference vector policy remains whole-file when present, otherwise the first stored section; that section is not guaranteed to represent a long note.

Semantic tools omit missing or unreadable source files and report partial-completeness reasons instead of returning blank successful matches. This checks source availability at validation time, not general embedding freshness or exact correspondence between stored passages and subsequently edited notes.
