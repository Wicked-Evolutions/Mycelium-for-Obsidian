...
- Added truthful exact-total, proven-limit, partial-scan, composite-result, and per-vault metadata to bounded file search, frontmatter query, analytics, cross-vault search, semantic search/similarity, and stale-index summaries. (#81)
...
- High-volume scans now distinguish complete-but-limited output from failed/skipped work, seek a qualifying text-search sentinel before claiming a bound, isolate cross-vault failures, and keep semantic sentinel candidates out of historical ranking, graph, and reranker pipelines. Legacy text-only filesystem readers remain format-compatible pending a separate success-response carrier. (#81)
- Semantic indexing now derives a conservative input budget from the installed Ollama model, deterministically splits long notes without dropping source content, verifies the source after both embedding and cache matches, and atomically replaces embeddings plus keyword rows. Exact cache reuse includes canonical paths, block IDs, FTS content, hashes, and model digest; provider, mixed-dimension, model-retag, or repeated source-change failures preserve the prior file index. Each embedding verifies the digest of the model Ollama loaded and the current tag mapping, with another tag check before commit or exact retrieval. Semantic retrieval searches one exact model-digest and vector-dimension cohort, rejects empty/nonfinite or identity-unverified provider vectors, excludes invalid or incompatible rows from vector and keyword ranking, and reports whether re-indexing is required. (#84)
- Semantic-index creation and publication now use an exact-pinned native adapter for a fixed set of descriptor-relative filesystem operations on macOS and Linux. Unsupported platforms keep the server and semantic-index-independent tools available, disable auto-indexing, and return `semantic_storage_unavailable` from semantic-index-dependent tools instead of using the pathname fallback. (#84)
...
- Obsidian CLI subprocesses that ignore the normal timeout or cancellation signal are now force-stopped after a bounded grace period instead of indefinitely stranding the request or test worker. (#84)
- Derived embedding storage now rejects symlinked or hard-linked index files and SQLite sidecars, reads the canonical database only through a verified handle, runs normal work against a private in-memory copy, and publishes mutations as securely created atomic snapshots. Older WAL-format main files are normalized through a private temporary directory outside the vault, serialized into memory, and receive an unconditional `finally` cleanup attempt. SQLite never opens canonical database or sidecar paths; live or stale legacy sidecars fail closed. Snapshot publication is serialized across processes, anchors every mutation and recovery cleanup to one pinned storage-directory descriptor, copies rollback bytes from a retained verified database handle into a private file, verifies the temporary inode through rename, and records a durable commit marker before cleanup. Authority verification and snapshot serialization share the same rejection boundary as journal publication: interrupted prepared publications restore the prior generation, committed publications preserve the new generation, rejected mutations restore the last published in-memory state, stale connections are invalidated, and missing or stale recovery artifacts fail closed. Parent-path replacement cannot redirect an operation into an external directory. Full-vault indexing publishes once at an operation-owned batch boundary, while watcher and single-file work waits behind the same per-vault queue. Upgrading from the prior WAL mode requires the older server process to stop first. Shared connections are bound to physical vault/directory/database identities and refuse old-generation reads or writes after same-path vault replacement. Watcher events are no longer stranded or conflated by colon-bearing vault/file names; deletions complete without Ollama, while transient provider failures receive bounded exponential retries without overwriting newer note events. (#84)

### Tests

- Added deterministic Unicode, literal-operator, hostile-punctuation, token-bound, zero-threshold, omitted-sort-field and near-tie ordering regressions. (#89)
...
- Added always-run injected-failure and mocked-provider coverage for exact-at-limit scans, qualifying sentinels, partial scans, duplicate semantic blocks, multi-chunk self exclusion, per-vault isolation, composite metadata, and unchanged semantic ordering/reranker inputs. (#81)
- Added deterministic long-note, UTF-8 boundary, heading-dense, exact-manifest cache, exact-model/dimension/finite-vector compatibility, mixed-dimension provider rejection, provider rollback, model-retag races across profile lookup, successive embeddings, commit, and query generation, SQLite transaction, failed-publication memory restoration across store/delete/clear/batch/close/reopen, pre-journal serialization rejection, stale-authority invalidation, cache/source races, physical storage authority, pinned-root replacement, verified snapshot reads, competing-publisher rejection, temporary-inode substitution rollback, abrupt pre-rename, post-rename/pre-commit, and post-commit publisher recovery, missing-rollback and removed-lock failure handling, operation-owned batch persistence, stale async-owner exclusion, symlink swap-back, rollback-copy pathname substitution, parent-directory mutation containment, hard-link and legacy-WAL sidecar escape, offline watcher deletion, bounded embedding-stage provider retry, serialized drain/key-collision, filesystem recovery, and real-Ollama long-note coverage. Clean-tarball verification now loads both native bindings and executes the installed secure-filesystem adapter. (#84)
- Added mocked ignored-timeout and early-abort-callback regressions plus a real SIGTERM-ignoring child-process cancellation test, and re-executed the real Ollama-gated semantic/reranker suite against a running local service. (#84)
